#!/usr/bin/env bash
echo "-----------------"

## Configurar caché de paquetes para acelerar isntalación
## echo "-----------------"
## echo "Añadiendo apt-cacher"
## echo "Acquire::http { Proxy \"http://192.168.1.254\";};" > /etc/apt/apt.conf.d/01apt-cacher-ng

echo "Actualizando repositorios y equipo"
apt update
#apt full-upgrade -y

echo "-----------------"
echo "Instalando mysql"

echo "mysql-server mysql-server/root_password password Abc123.." | debconf-set-selections
echo "mysql-server mysql-server/root_password_again password Abc123.." | debconf-set-selections

apt install net-tools mysql-client mysql-server -y

echo "[mysqld]" >> /etc/mysql/my.cnf
echo "bind-address = 0.0.0.0" >> /etc/mysql/my.cnf
systemctl restart mysql.service

echo "------------------"
echo "Creando base datos"
##mysql -uroot -pAbc123.. -h localhost -e "CREATE DATABASE wordpress;"
##mysql -uroot -pAbc123.. -h localhost -e "CREATE USER 'wordpress'@'192.168.56.21' IDENTIFIED BY 'Abc123..'";
##mysql -uroot -pAbc123.. -h localhost -e "GRANT ALL ON wordpress.* TO 'wordpress'@'192.168.56.21'";


echo "Creando base datos"
mysql -uroot -pAbc123.. -h localhost < traballadores.sql
mysql -uroot -pAbc123.. -h localhost < tendaBD.sql
echo "Creaando usuario acceso BD"
mysql -uroot -pAbc123.. -h localhost -e "CREATE USER 'userbd'@'%' IDENTIFIED BY 'Abc123..'";
mysql -uroot -pAbc123.. -h localhost -e "GRANT ALL PRIVILEGES ON *.* TO 'userbd'@'%'";
mysql -uroot -pAbc123.. -h localhost -e "flush privileges";
##mysql -uroot -pAbc123.. -h localhost < accesosusuarios.sql


echo "------ FIN ------"
